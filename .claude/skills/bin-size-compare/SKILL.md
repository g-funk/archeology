Compare naive vs tokenized byte sizes for both binary converters using test-data source files.

Run all four variants, using `/dev/null` as the output path so nothing is written to disk but byte counts are still reported:
